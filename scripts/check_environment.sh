...
if [[ -f /etc/os-release ]]; then
  ...
  if [[ "${ID:-}" == "ubuntu" && "${VERSION_ID:-}" == "20.04" ]]; then
    ok "Official target OS detected: Ubuntu 20.04 LTS"
  elif grep -qi microsoft /proc/version 2>/dev/null; then
    fail "WSL/WSL2 detected. Use an Ubuntu Server 20.04 LTS VM instead."
  else
    warn "This OS is not officially supported. Recommended: Ubuntu Server 20.04 LTS."
  fi
else
...
if python3 -c "import pandas, paho.mqtt.client" >/dev/null 2>&1; then
  ok "Python packages available: pandas, paho-mqtt"
else
  warn "Python packages pandas and/or paho-mqtt are missing in the current interpreter"
fi
...
COMPRESSED_DATASET_COUNT=$(find "$PROJECT_ROOT/devices" -type f \( -name "*.csv.xz" -o -name "*.txt.xz" -o -name "*.mp4" \) 2>/dev/null | wc -l | tr -d ' ')
if [[ "$COMPRESSED_DATASET_COUNT" -gt 0 ]]; then
  ok "Device dataset artifacts found: $COMPRESSED_DATASET_COUNT file(s)"
else
