...
# MQTT Topic
    hospital/nurse_stress

# Format
    X,Y,Z,EDA,HR,TEMP,id,datetime,label
    -0.123,0.456,0.789,1.23,72.5,36.5,nurse_01,2024-01-01 08:00:00,0
"""
...
from iotzoo_zenoh import publish
...
def telemetry(
    ...
    # Headers: X, Y, Z, EDA, HR, TEMP, id, datetime, label
    headers = ["x", "y", "z", "eda", "hr", "temp", "id", "datetime", "label"]
    units = ["g", "g", "g", "uS", "bpm", "C", None, None, None]
    ...
    while not die_event.is_set():
        if event.is_set():
            try:
                ...
                # Parse data types:
                # X, Y, Z - accelerometer (floats)
                # EDA - electrodermal activity (float)
                # HR - heart rate (float)
                # TEMP - temperature (float)
                # id - nurse identifier (string)
                # datetime - timestamp (string)
                # label - stress level (int/string)
                row[0] = float(row[0])   # X
                row[1] = float(row[1])   # Y
                row[2] = float(row[2])   # Z
                row[3] = float(row[3])   # EDA
                row[4] = float(row[4])   # HR
                row[5] = float(row[5])   # TEMP
                row[6] = row[6].strip()  # id
                row[7] = row[7].strip()  # datetime
                row[8] = row[8].strip()  # label

                data_dict = data2dict(headers, row, units=units)
                payload = as_json(data_dict)

                print(f"[telemetry] sending to `{mqtt_topic}': HR={row[4]} bpm, TEMP={row[5]} C, EDA={row[3]} uS, label={row[8]}")
